    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-100 pb-4">
        <div>
          <h1 className="text-xl font-black text-gray-900 flex items-center gap-2">
            <Truck className="h-5 w-5 text-[#e91e63]" /> Track Deliveries & Consignments
          </h1>
      ...
      {initialOrders.length === 0 ? (
        <div className="rounded-3xl border border-gray-200 bg-white p-12 text-center space-y-4 shadow-sm">
          ...
          <h2 className="text-base font-bold text-gray-900">No active consignments found</h2>
        ...
        <div className="space-y-6">
          ...
          {activeOrder && (
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 rounded-2xl bg-gray-50/70 p-4 border border-gray-100 text-xs">
                <div>
                  <span className="text-gray-400 font-medium block">Shipping Method</span>
                  <span className="font-bold text-gray-800">
                    {activeOrder.shipping_method || "Standard Courier (24-72h)"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-medium block">Payment Status</span>
                  <span className="font-bold text-emerald-700 capitalize">
                    {activeOrder.payment_status || "Cash on Delivery"}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400 font-medium block">Order Total</span>
              ...
              {activeOrder.order_items && activeOrder.order_items.length > 0 && (
                <div className="border-t border-gray-100 pt-4 space-y-2">
                  <span className="text-xs font-bold text-gray-700 block">Package Contents ({activeOrder.order_items.length} items):</span>
          ...
          {initialOrders.length > 1 && (
            <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm space-y-3">
              <h3 className="text-xs font-black uppercase text-gray-700 tracking-wider">Select Another Consignment to Track</h3>
                  ...
                  >
                    <div>
                      ...
                      <span className="text-[11px] text-gray-500">
                        {new Date(o.created_at).toLocaleDateString("en-GB")} • {formatPrice(o.total)}
                      </span>
                    </div>
